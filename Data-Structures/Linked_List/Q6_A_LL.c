...
[Step 1] 전체 순회하며 maxNode와 maxPrev 파악
- maxNode = [5]
- maxPrev = [2]

[Step 2] Max 노드 떼어내기 (maxPrev->next = maxNode->next)
- 2의 화살표를 5를 건너뛰고 3으로 바로 연결!
Head -> [2] ----------------> [3] -> [1] -> NULL
         (떨어져 나온 [5])

[Step 3] 떼어낸 Max 노드를 기존 맨 앞(Head)에 붙이기 (maxNode->next = *ptrHead)
[5] -> [2(기존 Head)] -> [3] -> [1] -> NULL

[Step 4] 리스트의 머리표표(Head)를 새로 갱신 (*ptrHead = maxNode)
Head -> [5] -> [2] -> [3] -> [1] -> NULL

[최종 결과]  
5 -> 2 -> 3 -> 1 -> NULL (제일 컸던 5가 맨 앞으로 이동 완료!)
===================================================================
*/
